...
- (void)locationManagerDidTimeout:(NSTimer*)aTimer userInfo:(id)userInfo
{
    ...
    [[LBSShopsAppDelegate sharedAppDelegate] alert:@"Location Service Error" message:@"Operation timeout"];
...
- (void)locationManager:(CLLocationManager *)manager didFailWithError:(NSError *)error
{
    ...
    if (!([error code] == kCLErrorDenied && [[error domain] isEqualToString:kCLErrorDomain])) {
        [[LBSShopsAppDelegate sharedAppDelegate] alert:@"Location Service Error" message:[error localizedDescription]];
    }
	...
	CLLocation * loc = [[CLLocation alloc] initWithLatitude:31.226735 longitude:121.557585];
	[delegate locationManagerDidReceiveLocation:self location:loc];
	[loc autorelease];
}
